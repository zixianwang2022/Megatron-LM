...
def add_text_generate_args(parser):
    ...
    group.add_argument('--nparts', type=int, default=8)
    group.add_argument('--part-id', type=int, default=0)
    ...
    #group.add_argument('--eval-split', type=str, default='evaluate split')
    return parser
...
def generate_samples_unconditional(model, visual_model):
    ...
    if args.eval_path.endswith(".npy"): # prepreocessed npy file
        ...
        eval_imgs, _test = [], []
        all_img_files = sorted(glob.glob(args.eval_path + "/*"))
        per_part = len(all_img_files) // args.nparts

        print('nparts: ', args.nparts)
        print('part_id: ', args.part_id)
        if args.part_id == args.nparts - 1:
            cur_img_files = all_img_files[per_part*args.part_id : ]
        else:
            cur_img_files = all_img_files[per_part*args.part_id : per_part*(args.part_id + 1)]
        num_samples = len(cur_img_files)
        pbar = tqdm(total=num_samples)

        for img_file in cur_img_files:
            if args.task == "OCR" and img_file.endswith(".png"):
                rgb_img = img_gray2rgb(img_file)
                rgb_img = image_transform(Image.fromarray(resize_with_pad(rgb_img, (args.img_h, args.img_w)))).numpy()

                img2 = Image.open(img_file)
                img2 = (ToTensor()(img2) * 255.).numpy().astype(np.uint8)
                #print(img2.shape, torch.sum(torch.abs(img2.reshape(-1))))
                raw_h, raw_w = img2.shape[1], img2.shape[2]
                ratio = float(max(args.img_h, args.img_w)) / max(raw_h, raw_w)
                new_h, new_w = int(raw_h * ratio), int(raw_w * ratio)
                transform = _transform_test(new_h, new_w)
                img2 = transform(np.transpose(img2, (1, 2, 0)))

                #print(img2.shape, torch.sum(torch.abs(img2.reshape(-1))))
                delta_h, delta_w = args.img_h - new_h, args.img_w - new_w
                top, bottom = delta_h // 2, delta_h - delta_h // 2
                left, right = delta_w // 2, delta_w - delta_w // 2
                img2 = torch.nn.functional.pad(img2, (left, right, top, bottom))
            else:
    ...
    while True:
        if torch.distributed.get_rank() == 0:
            ...
            if args.task == 'captioning':
                ...
                # sentences = ["Should you start and proceed through the intersection? Why?"]
            elif args.task == 'VQA':
            ...
            print(torch.sum(torch.abs(torch.from_numpy(eval_imgs[cnt]).reshape(-1))))
            ...
            token_embs = token_embs.transpose(0, 1)#.contiguous().bfloat16().cuda()
            
            #print(token_embs.shape, torch.sum(torch.abs(token_embs.reshape(-1))), eval_imgs[cnt].shape, np.sum(np.abs(eval_imgs[cnt][0].reshape(-1))))
            ...
            for prompt, generation in zip([prompt], resp_sentences):
                ...
                datum[generated] = generation[len(prompt):]
                print(datum)
                if answers:
                    print("Grountd truth: ", answers[cnt])
                yield datum
                cnt += 1
                pbar.update()
                if cnt >= num_samples:
...
def generate_and_write_samples_unconditional(model, clip_model):
    ...
    count = 0
    with open(args.genfile + f'_{args.part_id}', 'w') as f:
