...
echo ${run_cmd}
submit_job --gpu ${num_gpus} --nodes 4 --email_mode never  --mounts $MOUNTS --partition $PARTITION  --image $DOCKER -c "$LAUNCH ${run_cmd}" -n "${SAVENAME}" --duration 0.5  --exclude luna-0534,luna-0253,luna-0377 # --dependent_clones 1
