...
if [[ $model_card == *malachite_sawfly* ]]; then
    CHECKPOINT_PATH=${malachite_sawfly}
fi
if [[ $model_card == *neat_spoonbill* ]]; then
    CHECKPOINT_PATH=${neat_spoonbill}
fi
if [[ $model_card == *vehement_coyote* ]]; then
    CHECKPOINT_PATH=${vehement_coyote}
fi
if [[ $model_card == *wine_jackdaw* ]]; then
    CHECKPOINT_PATH=${wine_jackdaw}
fi
...
submit_job --gpu ${mod_par} --nodes ${pip_par} --email_mode never  --mounts $MOUNTS --partition $PARTITION --image $DOCKER  -c "$COMMAND" -n "generate_zeroshot_${model_size}_${TASK}" --duration 3
echo $COMMAND
# $COMMAND
