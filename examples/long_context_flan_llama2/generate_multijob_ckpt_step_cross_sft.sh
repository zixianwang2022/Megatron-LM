...
submit_job --gpu ${mod_par} --nodes ${pip_par} --email_mode never  --mounts $MOUNTS --partition $PARTITION --image $DOCKER  -c "$COMMAND" -n "generate_cross_${model_size}_${TASK}_${SAVENAME}_${use_retrieved_neighbours}" --duration 4 --exclude ${bad_nodes} --exclusive # --dependency afterany:4150880
