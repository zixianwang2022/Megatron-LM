...
bash ./tests/functional_tests/test_scripts/flamingo_ocr/finetune_flamingo_ocr_distributed_test.sh DATA_PATH=$DATA_PATH CHECKPOINT_PATH=$CHECKPOINT_PATH TENSORBOARD_DIR=$TENSORBOARD_DIR SCRIPTS_DIR=$SCRIPTS_DIR USE_TE=$USE_TE TP_SIZE=$TP_SIZE PP_SIZE=$PP_SIZE VP_SIZE=$VP_SIZE NUM_NODES=$NUM_NODES MAX_STEPS=$MAX_STEPS USE_CORE=$USE_CORE MBS=$MBS GBS=$GBS

python tests/functional_tests/python_test_utils/get_test_results_from_tensorboard_logs.py /lustre/fsw/adlr/adlr-nlp/jbarker/next-llm/output/flamingo_ocr_2b_SAM_test/tensorboard_logs flamingo_ocr_2b_SAM_test

export EXPECTED_METRICS_FILE=tests/functional_tests/test_results/flamingo_ocr/flamingo_ocr_2b_SAM_test.json

export LOGS_DIR=/lustre/fsw/adlr/adlr-nlp/jbarker/next-llm/output/flamingo_ocr_2b_SAM_test/tensorboard_logs
