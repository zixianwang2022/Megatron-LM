...
def flamingo_score_and_return_on_first_stage(model, tokens, lengths, vision_inputs):
    ...
    with torch.no_grad():
        ...
        #print(attention_mask[:5], position_ids)
        #print(stop)
        # logits will be meanigful only in the last pipeline stage.
        logits = forward_step(tokens, position_ids, attention_mask, vision_inputs)
...
def flamingo_beam_search_and_return_on_first_stage(model, tokens, lengths, beam_size, stop_token, num_return_gen, length_penalty, vision_inputs=None):
    args = get_args()
    tokenizer = get_tokenizer()
    batch_size = tokens.size(0)
    assert(batch_size == 1)
    ...
    with torch.no_grad():
        ...
        vision_inputs = vision_inputs.repeat(1, beam_size, 1)
        attention_mask, position_ids = _build_attention_mask_and_position_ids(tokens)
        prev_context_length = 0
