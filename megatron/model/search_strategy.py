...
from megatron.data.t5_dataset import make_attention_mask_3d, make_history_mask_3d
...
def forward_step_eval(batch, model):
    tokens_enc, y_block, enc_mask, dec_mask, enc_dec_mask, types, enc_hidden_states = batch

    logits, enc_hidden_states = model[0](tokens_enc,
                                         y_block,
                                         enc_mask,
                                         dec_mask,
                                         enc_dec_mask,
                                         ...
                                         return_enc_hidden_states=True)

    return logits, enc_hidden_states
...
class BeamSearch(object):
    ...
    def generate_output(self, model, tokens_enc, types, enc_mask):
        ...
        for i in range(self.max_decode_length):
            ...
            enc_dec_mask = make_attention_mask_3d(y_block, tokens_enc)

            batch = tokens_enc, y_block, enc_mask, dec_mask, enc_dec_mask, types, enc_hidden_states

            logits, enc_hidden_states = forward_step_eval(batch, model)
