...
#SBATCH -p batch --reservation=workload_validation -A llmservice_nlp_fm --nodes=16 --exclusive --mem=0 --overcommit --ntasks-per-node=8 --gres=gpu:8 --dependency=singleton --job-name=llmservice_nlp_fm-moe:3.5t-8x8b_upcycle_iter600000
...
NAME="3.5t-8x8b_upcycle_iter600000"

DIR=`pwd`
DATETIME=`date +'date_%y-%m-%d_time_%H-%M-%S'`

INIT_CHECKPOINT_DIR="$OUTPUT/3.5t-8x8b_upcycle_middle"
...
# Get the data blend
. nvllm-3.5t-eng-v0.2-mul-a1.3-code-a1.3-wdata-nojsp-blend-v0.701515-localized.sh
...
options=" \
	...
	--tokenizer-model $ADLR_SHARING/nvllm-3.5t/utils/mt_nlg_plus_multilingual_ja_zh_the_stack_frac_015_256k.model \
...
srun -l \
     --container-image "${SQSH}/24.01.sqsh" \
