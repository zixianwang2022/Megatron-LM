...
def context_generation(input_list, list_of_topk_list, num_prompt_examples=0, gen_model=None, megatron_api_url=None, multiple_gen=1):
    ...
    while True:
        ...
        for i in range(multiple_gen):
            ...
            if megatron_api_url is not None:
...
def construct_input_prompt_ours(input_list, prompt_data, ctx_generation_list=None, retriever=None, model=None, \
    ...
    if args.is_context_generated:
        ...
        if ctx_generation_list is not None:
            ...
            context_current_list = context_generation(input_list, list_of_prompt_sample_list, args.num_prompt_examples, model, args.megatron_api_url, multiple_gen)
    else: 
    ...
    and mpu.get_tensor_model_parallel_rank() == 0:
        ...
        if os.path.exists(output_file) and os.path.getsize(output_file) > 0:
            with open(output_file, "r") as fin:
                ...
                input_pos = len(current)
                print("starting from the position {}!".format(input_pos), flush=True)
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            if args.openai_api:
                ...
                prompts_plus_generations_list = outputs
                ...
                for context_generation in context_current_list:
                ...
                if args.beam_search:
                    ...
                    # print("*"*10, flush=True)
                    # print(prompts_plus_generations_list[0])

                # write the generated output to the output file
                if mpu.get_tensor_model_parallel_rank() == 0:
                    if mpu.is_pipeline_first_stage():                            
                        for prompts_plus_generations, raw_text_len in zip(prompts_plus_generations_list, \
                                                raw_text_len_list):
                            generations = prompts_plus_generations[raw_text_len:].strip()
...
def debug(model):
    ...
    with torch.no_grad():
        if mpu.is_pipeline_first_stage() \
            and mpu.get_tensor_model_parallel_rank() == 0:
                prompt_text_list=["THIS IS NOT EMPTY TEXT"]
        else:
            prompt_text_list=["EMPTY TEXT"]

        print("======samples=====!", flush=True)
        print(prompt_text_list, flush=True) 
        print("rank is {}".format(torch.distributed.get_rank()), flush=True)                                
