...
#SBATCH -p batch_dgx2h_m2 --nodes=2 --gres=gpu:8 -A gpu_adlr_nlp -t 0:30:00 --exclusive --nv-meta=ml-model:language-modeling --job-name=adlr-nlp-dev:retro --ntasks-per-node=4
...
# >>>
# [selene] SBATCH -p luna -A adlr -t 4:00:00 --nodes=1 --exclusive --mem=0 --overcommit --ntasks-per-node=1 --job-name=adlr-nlp-dev:retro
# [draco-rno] SBATCH -p batch_dgx2h_m2 --gres=gpu:16 -A gpu_adlr_nlp -t 8:00:00 --exclusive --nv-meta=ml-model:language-modeling --job-name=adlr-nlp-dev:retro
...
LOG_DIR="./retro/build/logs"
mkdir -p $LOG_DIR

run_cmd="pwd && cd $SHARE_SOURCE/megatrons/megatron-lm-retro-index-add && pwd && bash retro/build/build_index_inner.sh"
