...
if __name__ == '__main__':
    ...
    description='Converts a checkpoint to Granular MoE')

    parser.add_argument('--input_dir', type=str, required=True)
    parser.add_argument('--output_dir', type=str, required=True)
    parser.add_argument('-E', '--expansion', type=int, required=True)
    parser.add_argument('-G', '--granularity', type=int, required=True)
    parser.add_argument('--transformer_impl', type=str, default='grouped_gemm')
    parser.add_argument('--router_std', type=float, default=0.1)
    ...
    print("Expansion Factor= "+str(E))
    print("Granularity= "+str(G))
    print("Number of Experts= "+str(num_experts))
        ...
        for k, v in state_dict['model'].items():
            ...
            if m:
                ...
                if not (layer_num in routers):
                    routers[layer_num] = torch.nn.Linear(v.size(1), num_experts)
                ...
                # Create the fc1 weights
                assert(args.transformer_impl == 'grouped_gemm') # haven't implemented anything else yet
                new_key = 'decoder.layers.'+m.group(1)+'.mlp.experts.weight1'
                assert(v.size(1)%G==0) # hidden size must be divisible by G
                new_key_values.append((new_key, v.detach().clone().t().repeat(E, 1, 1).reshape(v.shape[1], -1).contiguous()))
                old_keys.append(k)
                continue
            ...
            if m:
                ...
                assert(v.size(0)%G==0) # hidden size must be divisible by G
                new_key_values.append((new_key, v.detach().clone().t().repeat(E, 1, 1).reshape(-1, v.shape[0]).contiguous()))
                old_keys.append(k)
                continue
